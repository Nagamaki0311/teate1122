...
| T-023 | バグ修正: ヒーロー本文テキストが中央配置されず左寄りになる | 中 | レビュー中 | developer | User報告（「夜のはじまりに、ひとつだけ火を灯す。〜」の表示位置が左寄り）。Manager診断（Playwright実機診断・原因特定・修正案の動作確認）をDeveloperが引き継ぎ正式適用。**原因**: `src/style.css`の`.hero-section__body`が`max-width:24em`で幅を制限されているにもかかわらず`margin:30px 0 0`（左右autoなし）のため、親`.hero-section__inner`（`margin:0 auto`で中央配置）の中でブロック自体が左端に寄っていた。`data-align="center"`による`text-align:center`は行内テキストを段落ボックス内で中央寄せするだけで、段落ボックス自体の水平位置には影響しないため発生。**修正**: `margin: 30px 0 0;`を`margin: 30px auto 0;`に変更する1行のみ。`npm test`91件・`npm run build`グリーン。Playwrightで再検証済み（1440×900・390×844ともbodyCenter=innerCenter、diff=0。修正前はdiff=-336px）。**他セクションの位置確認**: 全セクション（hero/philosophy/profile/candles/events/gallery/contact）をDeveloperが独立にPlaywrightで全数再スキャンし、hero以外に問題なしを確認（Managerの診断結果と一致）。D-029参照。Reviewerによるレビュー待ち。 |
