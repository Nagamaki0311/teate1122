...
| T-021f | バグ修正: ヒーローSCROLL表示を削除（表示形式によらず再発するため） | 中 | レビュー中 | developer | User実機報告（T-021e修正後も、スマホ通常表示では窮屈、「PC版サイトを見る」モードでは再度崩れる）。Userの提案に従いSCROLLインジケーター自体を削除する方針（Manager判断）。`src/_includes/sections/hero.njk`と`src/style.css`から`.hero-section__scroll`関連を削除。T-021eで追加したモバイル幅`min-height`メディアクエリはPlaywright実測（維持/削除の比較）の結果、背の低い機種（375×667）で余白が71.6px→15.6pxまで縮むため維持と判断。Playwrightで複数ビューポート実機確認済み（SCROLL非表示・本文と`.mobile-tabs`が重ならないことを確認）。D-027参照。 |
| T-022 | トップエリア（ヒーロー）背景アニメーション実装（灯火の光の粒子） | 中 | 計画中 | planner | Planner計画完了。**方式: CSS `@keyframes`のみ採用**（Canvas/tsParticles等ライブラリは不採用、新規依存なし）。`hero.njk`の`__scrim`と`__inner`の間に`.hero-section__embers`（span×14、nunjuck `range`ループ）を追加。漂流アニメーション（16〜34秒・粒子毎に負のdelay）と明滅アニメーション（3.2〜7.4秒）を重ねて不規則感を出す。色は`--color-ember-glow:#ffd98e`をトークン新設。透明度上限0.5・配置はヒーロー上部〜中部（`--y`6〜68%）に限定しテキストと重ならないようにする。モバイル（`max-width:819px`）は`:nth-child(n+9){display:none}`で8個に削減。ビューポート外での停止は`site.js`に新規`IntersectionObserver`を追加し`[data-embers-paused]`属性でCSS側`animation-play-state:paused`に連動。`prefers-reduced-motion:reduce`では完全非表示（既存メディアクエリに追記）。端末性能検知（hardwareConcurrency等）は描画コストが軽微なため不採用と判断。T-021aバックログの「ttdrift」（背景写真自体のゆっくりしたズーム/パン、粒子とは別物）は今回対象外・バックログ継続。**Userへの確認事項はPlannerの判断で最小化済み（色味・密度の1点のみ、実装後スクリーンショット提示时に確認）**。**T-021fのmainマージ・ローカルmain同期後にDeveloper着手。** 変更ファイルは`hero.njk`/`style.css`/`site.js`/docs一式の4種のみ、editor側は`?raw`参照のため無変更で自動追従。Developer着手時の注意: D-027はT-021fで使用済みのため、本タスクのADRは**D-028**から採番すること。 |
