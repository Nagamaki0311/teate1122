...
## D-027: T-021f、ヒーローSCROLLインジケーターの削除（D-026の修正後もUser実機で問題が再発したため）

- 日付: 2026-09-10
- 状態: 採用

### 背景
- D-026（T-021e）でSCROLLインジケーターと`.mobile-tabs`の重なりを修正しマージ済みだったが、User実機確認の結果、以下の2点が判明した。
  1. 通常のスマホ表示では見切れなくなったものの、右下ぎりぎりの配置で窮屈に見える。
  2. スマホブラウザの「PC版サイトを表示」モード（デスクトップ幅として描画される）では、既知の別バグ（`.mobile-tabs`がCSSカスケード順序の問題でデスクトップ幅でも非表示にならず表示され続ける。`src/style.css`、T-021eバックログに記録済み）と絡んで再度SCROLL表示が崩れる。
- Userから「どの表示形式でも画面中央の適切な配置になるよう修正、またはSCROLLの表示自体を削除してほしい」と要望があり、Manager判断でSCROLLインジケーター自体の削除を選択した。複数の表示モード（通常モバイル幅／`.mobile-tabs`カスケードバグが影響するデスクトップ幅の両方）で確実に破綻しない、最も安全な選択のため。SCROLLインジケーターは装飾的なスクロール誘導であり必須要素ではなく、削除しても情報の欠落は生じない。

### 決定・修正内容
1. `src/_includes/sections/hero.njk`から`.hero-section__scroll`のブロック（`SCROLL`テキスト＋縦線`.hero-section__scroll-line`）を削除した。
2. `src/style.css`から`.hero-section__scroll` / `.hero-section__scroll span:first-child` / `.hero-section__scroll-line`の3ルールを削除した。
3. D-026で追加した`@media (max-width: 819px) { .hero-section { min-height: calc(...) } }`（SCROLLインジケーターと`.mobile-tabs`の重なり回避が目的だった）について、SCROLL削除により存在理由が本質的になくなった可能性を検討し、削除した場合と維持した場合の両方でPlaywright実測を行った（詳細は下記「検証方法」）。結果、通常のモバイル幅（390×844等）や819px境界では削除してもヒーロー本文と`.mobile-tabs`は重ならなかったが、より背の低いビューポート（375×667、iPhone SE相当）ではメディアクエリを維持した場合の余白71.6pxに対し、削除した場合は15.6pxまで縮まることを確認した。実機のブラウザUI（アドレスバーの表示/非表示による可視高さの変動）や機種差を考えると15.6pxの余白は将来的な重なりリスクが高いと判断し、**このメディアクエリは維持する**ことにした（Ponytail「不要になった対応は消す」の原則は、実測で本当に不要と確認できた場合にのみ適用する。今回は実測の結果、削除すると別の余白リスクを生むため維持が妥当と判断）。コメント文中のSCROLLインジケーターへの言及のみ削除し、`.mobile-tabs`重なり回避という現行の目的を明記するよう更新した。
4. プレースホルダーSVG（`src/assets/hero.svg`、D-026で`y=250`に変更済み）については、SCROLL削除により制約条件（SCROLLとの重なり回避）は不要になったが、見出しとの重なり回避という制約は引き続き必要であり、`y=250`は画像空間上部の余白（見出し帯・円形グラデーションのいずれよりも上）に位置しているため変更は不要と判断した（実際にPlaywrightスクリーンショットで見出しと重ならないことを確認済み）。

### 検証方法
- `npm run build`後の`_site`をローカル配信し、Playwright（Chromium、`/opt/pw-browsers/chromium-1194`、`--ignore-certificate-errors`）で以下のビューポートをスクリーンショット・DOM実測した。
  - 390×844（モバイル標準）
  - 375×667（iPhone SE相当、背の低いモバイル）
  - 819×844・820×900（`.mobile-tabs`表示切替の境界値。1440×900と同様、既知バグにより実際は両方とも`.mobile-tabs`が`display:flex`のまま表示される）
  - 1440×900（デスクトップ。スマホの「PC版サイトを表示」モードの簡易的な代替として、単純にビューポート幅を広げたテストで代替した）
- 全ビューポートで`.hero-section__scroll`要素がDOMに存在しないこと（`document.querySelector`が`null`）を確認した。
- `.hero-section__body`の`getBoundingClientRect().bottom`と`.mobile-tabs`の`getBoundingClientRect().top`を比較し、重なりがないこと（`overlapBodyVsTabs: false`）を全ビューポートで確認した。
- メディアクエリ維持・削除の両パターンで上記を実測し比較した実測値（`.hero-section__body`下端と`.mobile-tabs`上端の差＝余白）:
  - 390×844: 維持時114.1px / 削除時58.1px
  - 375×667: 維持時71.6px / 削除時15.6px
  - 819×844: 維持時114.1px / 削除時58.1px（幅819pxは`max-width:819px`の対象内）
  - 820×900・1440×900: メディアクエリの対象幅外のため維持・削除で差はなし（いずれも72px弱の余白）
- なお、320×568（旧世代iPhone SE等、極端に小さい機種）ではメディアクエリ維持・削除どちらでも本文と`.mobile-tabs`が約22px重なることを実測したが、`git stash`でT-021f着手前のコード（D-026マージ後の状態）に戻して同条件を再現したところ同じ重なりが再現したため、本タスクの変更に起因するものではなく既存の別問題と判断し、バックログに追加するに留めた（本タスクのスコープ外）。
- `npm run build`が成功することを確認済み。

### 影響
- SCROLLインジケーターの表示・関連CSSが完全になくなる。装飾的なスクロール誘導表示が一つ減るのみで、機能的な欠落はない。
